...
7. **Stadio F persone** (solo totale 55+): titolare o buyer con ruolo da sito, bio IG, LinkedIn pubblico, registro imprese; email nominativa solo se pubblicata; email generica pubblicata se non c'e' di meglio. In `lead_contacts` con `fonte`, via `session_write.mjs`. Email dedotte solo in `note` con `[DA VERIFICARE]`. **Se sito e IG non hanno email, la pagina Facebook del negozio quasi sempre ce l'ha**: in Chrome `facebook.com/search/pages/?q=<nome citta'>`, poi `facebook.com/<pagina>/directory_contact_info/` (il 25-09 ha dato l'email a 6 A/B su 8 provati, spesso quella nominativa del titolare).
