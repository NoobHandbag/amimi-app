-- 0021_on_shopify_live
-- #5 fix: on_shopify was joined to the STALE seed `shopify_catalog` (old accessory color
-- names: PINK_OLIVE, MUSTARD_ROSE, REED...). The gestionale + the LIVE Shopify sync
-- (`shopify_stock`, refreshed daily) already use the new names (CANDY_PINK, LIME_PINK, RED...).
-- "Shopify wins" => derive on_shopify from the live shopify_stock, matched on codice_norm.
-- Only the on_shopify source changes; all output columns/types/order stay identical so
-- dependent views (v_shopify_align, v_sku_availability, v_reorder) keep working.
CREATE OR REPLACE VIEW v_inventory AS
 WITH pur AS (
         ...
         SELECT DISTINCT upper(regexp_replace(COALESCE(shopify_stock.codice, ''::text), '\s+'::text, '_'::text, 'g'::text)) AS codice_norm
           FROM shopify_stock
