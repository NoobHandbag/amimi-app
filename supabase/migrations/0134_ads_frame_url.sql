-- 0134: fotogramma VIDEO piu' grande, risolto lato server (owner 2026-09-20).
-- Fino a 0133 i video mostravano solo thumbnail_url (crop 64x64: renderizza ma minuscolo/sfocato). L'edge ads-sync
-- ora risolve un fotogramma GRANDE per i video (poster del video, o il thumbnail piu' grande dal nodo video) e lo
-- salva in meta_ad_creative.frame_url. Il frontend usa frame_url quando c'e', altrimenti image_url (foto) e infine
-- thumbnail_url (64px). Additivo: colonna nuova + ridefinizione della sola vista che la espone.
alter table meta_ad_creative add column if not exists frame_url text;

-- v_ads_creative_status: come 0133 ma con c.frame_url nel select (drop+create perche' cambia l'elenco colonne;
...
create view v_ads_creative_status as
 with x as (
   select w.ad_id, w.as_of,
     ...
     c.object_type, c.thumbnail_url, c.image_url, c.frame_url, c.link,
