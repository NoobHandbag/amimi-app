...
- **Deploy edge/migrazioni** (solo Claude Code, Regola 16): edge con la **CLI**, `npx supabase functions deploy <nome> --project-ref imszbjeyplaiovylhkgl --no-verify-jwt --workdir <cartella amimi-app>` (aggiornato 21-09, quest audit Premia: le tre edge Premia hanno `verify_jwt = false` pinnato in `supabase/config.toml`, sezione `[functions.<nome>]`, cosi' un deploy senza il flag non le spegne piu'; per TUTTE le altre edge il flag esplicito resta obbligatorio, e un redeploy senza flag rimette `verify_jwt` a `true`). Sempre, nell'ordine: git backup su branch (commit + push; policy owner dal 18-09: mai su `main` diretto, PR verso `main`), misura Regola Ferrea 19 PRIMA (CE del mese corrente e stock: query in `Cowork12/projects/Premia_Area_Membri_2026-09/07_AUDIT_0_inventario.md` §8), deploy, `npx supabase functions download <nome>` e `diff` col repo per leggere il **bundle vivo** (mai il numero di versione), misura Regola 19 DOPO (identica). Migrazioni in `supabase/migrations/` via `apply_migration`, con nome migrazione = nome file. Frontend: `cd web && npm run build && npm run deploy` (hard refresh della PWA dopo), SOLO da un HEAD che contiene `origin/main`: dal 22-09 il `predeploy` si rifiuta altrimenti (`AMIMI_DEPLOY_ALLOW_BEHIND=1` per lo sblocco esplicito), dopo che un deploy da un branch vecchio ha sovrascritto i fix appena pubblicati da main. Prima di pubblicare leggere `version.json` live e dire da quale commit si riparte. Runbook del modulo Premia (go-live, rollback, pool, backfill, storno, incidenti, calendario deprecazioni): `docs/LOYALTY_RUNBOOK.md`.
