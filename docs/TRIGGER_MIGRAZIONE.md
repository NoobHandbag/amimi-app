...
| 5 | **Ruotare i segreti**: 3 Supabase (`gemini_api_key`, `mcp_token`, `qromo_webhook_secret`) + **token Shopify** (esposto in chat 01-07). ATTENZIONE (scoperto 04-07): la `?key=` del webhook Qromo e' anche nella STORIA git PUBBLICA di amimi-app (commit 91a71ac-d4327dd; rimossa dall'albero, la storia resta). Piano concordato con owner: DOPO il smoke test della prima vendita, rifare il webhook in console con `?key=` nuova mai committata + aggiornare `qromo_webhook_secret` e `qromo_webhook_token` in app_flags. Fino ad allora la chiave trapelata resta valida: priorita' alta | 🔴 da fare dopo smoke test |
