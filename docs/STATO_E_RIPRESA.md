...
9. **Aperti post-remediation audit 2026-07-06** (lo stato VIVO e' nella testata di `audits/AUDIT_SYSTEM_2026-07-06.md` e nelle voci del 06-07 di `_CHANGELOG_CODE.md`): rotazione segreti + chiusura esfiltrazione ask_select (A1/A2, URGENTE, saltata per scelta owner: esposizione ancora live); smoke test webhook Qromo (0 righe `source='qromo-direct'` al 06-07, baseline 150); attivazione uptime monitor esterno (`.github/workflows/uptime.yml` creato ma il push richiede lo scope `workflow`: azione owner); diferiti B13 (idempotency-token) e B16 (atomicita' arrival); period_mismatch=1 (spesa 2025, lasciata all'owner). I mesi chiusi sono stati corretti e RI-CHIUSI il 06-07: `v_ce_drift` a 0.
