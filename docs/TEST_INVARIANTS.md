...
| **Non coperto** | i pin elencati nel referto (sez. "Gate 3"): `write_api_guardie`, `write_api_num`, `date_roma`, `retry_once`, `shopify_resolve`, `qromo_item`, `autopush_decision`, `schema_parity`, `backup_copertura`, `tracked_no_pii`, estensioni di `features.mjs` | si scrivono insieme ai fix che chiudono i finding, non prima: un pin senza fix e' un rosso permanente, non una guardia |
