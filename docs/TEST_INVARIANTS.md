...
| **Test** | `tests/ads_sync_guardie.mjs` (27 asserzioni: A1 con autocontrollo di `redact_`, B3-B7 con replica PURE di `romeDate_` sul cambio ora 28-03-2027, B1/B2/C2/C7 sulle migrazioni, v1.1 resolver via `shopify_stock.variant_id`); voce `ads-sync` in `tests/letture_controllate_guardie.mjs` (11 asserzioni) | girano nella Gate 1 della skill; da aggiungere alla Action quando il token gh avra' lo scope `workflow` |
