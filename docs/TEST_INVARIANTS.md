...
| **Test** | `tests/loyalty_guardie.mjs` (arriva con i fix di Fase 6, mai prima: un pin senza fix e' un rosso permanente) | frammenti attesi dei fix L1-L9, T1, T2, T9 e regex "lettura senza error" sui rami `rewards`/`redeem` |
| **Non coperto** | comportamento dell'Admin API sotto la versione pinnata (`2026-07`, Fase 6) e concorrenza reale su `award` | si collaudano dopo il fix sull'account di test (`07_AUDIT_1` L3, `07_AUDIT_2` T20) |
