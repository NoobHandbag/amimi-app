...
| **Test** | `tests/loyalty_guardie.mjs` (83 pin sul sorgente, job `test` bloccante, dal 2026-09-21 con i fix di Fase 6 e dal 22-09 con i pin 44-73 della migr 0141 e di `loyalty-proxy` v12) | frammenti attesi dei fix: API `2026-07` e niente `2024-01` (T20), firma fresca (T1), PIN (T2), epoch/lotti/finestra (L1/L2/L8), RPC `loyalty_award_daily` e niente `award()` (L3), claim idempotente e idemp del proprietario (L4/L6, migr 0136), storno da due fonti (L5/G2), cinture di schema (T16), `rewards` con errori controllati (T9), guardaroba dal DB su `codice_norm` (S1/T33), escape e rete della pagina, `verify_jwt` in `config.toml`, una sola 0135/0136/0137 |
