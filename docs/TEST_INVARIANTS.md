...
| **Test** | `tests/shopify_sync_guardie.mjs` (41 asserzioni), `tests/letture_controllate_guardie.mjs` (135 asserzioni su 11 funzioni) | sul SORGENTE, non a runtime: la classe compare solo quando il database risponde male, e un collaudo non lo fa mai. I frammenti attesi sono quelli dei fix del 13-09 (un agente per funzione, rivisto da un revisore indipendente; per `loyalty-proxy` riallineati il 22-09 alla v11 del 21-09, che aveva cambiato 4 frammenti: il test era vecchio, il codice giusto), piu' una regex globale che vieta `retryOnce` su insert/update/upsert/delete non di telemetria. Dal 14-09 (audit gate, finding B57) le catene supabase-js che vanno a capo prima di `.select(`/`.upsert(` vengono riattaccate prima delle regex, e quattro asserzioni di autocontrollo provano che il rilevatore vede un retry su scrittura anche scritto su due righe (prima passava a vuoto) |
...
## 15. Il cancello periodico: build, audit, pin (dal 2026-09-14)

| Cosa | Dove | Nota |
|---|---|---|
| **Invariante** | tutto il repo | una modifica larga o un audit periodico passa tre cancelli: Gate 1 build offline (le suite di questa tabella + `tests/edge_syntax.mjs` su tutte le edge + build e lint del web; `node --check` e' stato tolto il 22-09 perche' cieco sui `.ts` con `import`), Gate 2 audit a effort alto con un refuter indipendente per ogni finding A/B, Gate 3 pin (ogni finding chiuso o rinunciato per iscritto e fissato da un test). Skill `~/.claude/skills/amimi-audit-gate`; referto del primo giro completo `audits/AUDIT_GATE_2026-09-14.md` (14 aree, 138 finding: 7 A, ~75 B, ~55 C) |
| **Test** | `.github/workflows/amimi-app-tests.yml` (Gate 1 a ogni push su edge, migrazioni, snippet, test, web; ancora da pushare: serve lo scope `workflow` sul token gh) | la Action gira le suite in un loop `node suite || fail=1` con `shell: bash` e senza pipe: la prima stesura faceva `node suite | tail -3` con `set -e` e senza pipefail, cioe' un test rosso restava verde (finding B56) |
| **Fatto (blocco 1, 14-09)** | `tests/write_api_num.mjs` (num/isoDate/todayRome/ymFromIso/tok/cnorm/VALID_CATEGORIE su `write-api/lib.ts`), `tests/date_roma.mjs` (stessa data di Roma e stesso tok fra server e web), `tests/write_api_guardie.mjs` (le cinture di write-api v26 sul sorgente), piu' le asserzioni health_log-upsert in `tests/shopify_sync_guardie.mjs` | scritti insieme ai fix del blocco 1; girano nella Action e nella Gate 1 della skill |
| **Fatto (22-09, lead-outreach)** | `tests/lead_outreach_guardie.mjs` (50 asserzioni: blocco `PURE:lead-guard` ritagliato dal sorgente + regole su sorgente e migr 0139) | segnaposto rimasti, destinatario, opt-out, firma/opt-out garantiti, avvisi di contenuto (cotone/Italia, condizioni, allegati), giorno di Roma per il tetto, claim prima di Gmail, vincoli unici a DB, esito incerto e sblocco. Nella Gate 1 della skill dal 22-09 |
| **Fatto (22-09, deploy web)** | `web/scripts/check-bundle-env.mjs` (predeploy): quarto anello, `git merge-base --is-ancestor origin/main HEAD` | un deploy da un HEAD che non contiene `origin/main` si ferma (22-09: un branch partito dal main vecchio ha sovrascritto il sito per 5 minuti). Sblocco esplicito `AMIMI_DEPLOY_ALLOW_BEHIND=1`, da scrivere nel changelog. Provato a mano il 22-09: HEAD = origin/main passa, un worktree staccato su a3cca50 si ferma |
| **Fatto (22-09, sintassi edge)** | `tests/edge_syntax.mjs` (parser TypeScript di Node su tutte le `index.ts` + `write-api/lib.ts`, con autocontrollo) | SOSTITUISCE `node --check` nella Gate 1: con Node 24 `node --check` su un `.ts` che contiene un `import` esce 0 anche su un file rotto (misurato il 22-09 su un edge con apici non escapati). Al 22-09: 24 file, 0 rotti |
| **Fatto (23-09, materie prime Fase 2)** | `tests/mat_guardie.mjs` esteso a 62 (56-62 = fix della Gate 2 del 23-09: chiave nell'header e scrub, num migliaia, upload per identita', foto di documento, righe non attive, offerte doppie, tetti del bucket) (migr 0144: log AI senza privilegi, upload solo sotto inbox/, flag OFF; mat-api: JWT, flag con errore controllato, scritture solo su mat_* e log, audit, 23505, niente retry su scritture; ai-compila: scrive solo il suo log, JSON mode senza thinkingConfig, tetti e timeout, 503 su errore; blocchi PURE ritagliati: num/chiDa/dataOk/ilikeEsc e buildPrompt/normalizza); `tests/mat_ai_golden.mjs` = accettazione dello strato AI sui documenti reali (a mano, con la chiave) | il golden set e' l'unica misura vera dello strato AI: un numero proposto dove il documento non lo ha = rosso |
| **Non coperto** | i pin dei blocchi successivi: `retry_once`, `shopify_resolve`, `qromo_item`, `autopush_decision` (A3), `schema_parity`, `backup_copertura` (A7), `tracked_no_pii` (A6), estensioni di `features.mjs` (identita' `v_inventory`, spedizioni nel Totale) | si scrivono insieme ai fix che chiudono i finding, non prima: un pin senza fix e' un rosso permanente, non una guardia |

## 16. Il segreto di un servizio non finisce in un log leggibile, e una regola che non puo' scattare non e' una regola (dal 2026-09-18)

Nate dal Gate 2 della feature Amimì Ads (edge `ads-sync`, migr 0128-0131): il revisore ha bloccato la prima versione su un token Meta che, in querystring, sarebbe finito in `health_log.label` (leggibile da anon) al primo rigetto di `fetch`, e su letture di `products`/`product_aliases` con l'errore ignorato che avrebbero sbiancato `codice_norm` su tutte le righe dichiarando "ok". Piu' una regola di fatica con soglia `frequency >= 3` su medie GIORNALIERE (max reale 2,1): irraggiungibile, cioe' morta.

| Cosa | Dove | Nota |
|---|---|---|
| **Invariante** | `ads-sync` | il token viaggia SOLO in header e ogni messaggio verso `health_log` o in risposta passa da `redact_`; le letture da cui dipende cio' che si scrive (`products`, `product_aliases`, `shopify_stock`) destrutturano `error`, ritentano una volta e fermano il giro (Regola 20), con guardia sul cap 1.000; un pull troncato a `maxPages` e' un errore; dedup prima di ogni upsert; date di Roma con `Intl`, mai `toLocaleString` |
| **Invariante** | `v_ads_creative_status` (0130) | le soglie di fatica sono raggiungibili sui dati reali (alta >= 1,7 con max osservato 1,74 sulla media a 7g) e "set scoperto" richiede copertura (>= 3 risolti e >= meta' del set) e dice su quanti prodotti si basa |
| **Test** | `tests/ads_sync_guardie.mjs` (40 asserzioni: A1 con autocontrollo di `redact_`, B3-B7 con replica PURE di `romeDate_` sul cambio ora 28-03-2027, B1/B2/C2/C7 sulle migrazioni, v1.1 resolver via `shopify_stock.variant_id`; 20-23b redesign freq7 vera/image_url/viste per modello/status v2; 24-28 fotogramma video lato server: `frame_url` dal poster `video_data`/`asset_feed_spec` o dal nodo video, migr 0134); voce `ads-sync` in `tests/letture_controllate_guardie.mjs` (11 asserzioni) | girano nella Gate 1 della skill; da aggiungere alla Action quando il token gh avra' lo scope `workflow` |
| **Collaudo a mano** | 18-09: `health` (account letto), 4 backfill (14/30/30/16 giorni, 0 errori, 623 righe), verifica su `v_ads_creative_status`/`v_ads_set_inventory`/`v_ads_weekly_account` con dati reali | il comportamento contro Meta (rate limit, forme delle actions, permessi catalogo) NON e' collaudabile offline: e' un gap dichiarato, coperto dal collaudo manuale e dalla telemetria |
| **Non coperto** | frequency vera a 7 giorni (reach deduplicata): oggi `freq_media_giornaliera_7` e' una proxy dichiarata; parita' `schema.sql` vs live per le tabelle nuove | si scrivono insieme ai fix, non prima |

## 17. Il modulo Premia e' quadrato, riproducibile e a prova di deploy distratto (dal 2026-09-21, quest audit Area Membri)

Nate dalle Fasi 0-5 dell'audit (`Cowork12/projects/Premia_Area_Membri_2026-09/07_AUDIT_*`). Il modulo aveva un solo membro (l'account di test) e nessun cliente reale: l'audit ha usato quel vuoto per misurare tutto prima del lancio.

| Cosa | Dove | Nota |
|---|---|---|
| **Invariante** | tabelle `loyalty_*` | `loyalty_points.points == SUM(loyalty_events.delta)` per ogni membro: vista `v_loyalty_ledger_drift` (migr 0137) e allarme `loyalty_ledger` in `health_log` ogni mattina alle 06:12 UTC (dopo `refresh_health_log()` delle 06:00, che cancellerebbe le chiavi `loyalty_*` del giorno). Verificato 0 drift il 20 e 21-09 |
| **Invariante** | RPC `loyalty_credit_order`, `loyalty_redeem`, `loyalty_claim_code` | ogni accredito e detrazione e' una RPC atomica con insert-gate a DB (PK ordine, UNIQUE idemp, guardia `points >= cost`); il pool non consegna mai due codici alla stessa redemption e il ramo `already` risponde solo al cliente proprietario (fix migr 0136, Fase 6) |
| **Invariante** | `supabase/functions/loyalty-page` | `index.ts` e' GENERATO da `src/index.html` con `build.mjs`: `node supabase/functions/loyalty-page/build.mjs --check` e' rosso se divergono (Gate 1). Il file deployato v4 del 20-09 e' stato riprodotto byte a byte dalla sorgente il 21-09 |
| **Invariante** | `supabase/config.toml` `[functions.loyalty-proxy\|loyalty-orders\|loyalty-page] verify_jwt = false` | un `functions deploy` senza `--no-verify-jwt` non spegne piu' l'App Proxy ne' la pagina (T24) |
| **Misura** | Regola Ferrea 19, prima e dopo OGNI deploy del modulo | `v_ce_totale_summary` del mese corrente (`omni_netto`, `mc2`) e `sum(giacenza_attuale)`, `sum(valore)` di `v_inventory` devono restare identici: baseline e query in `07_AUDIT_0` §8, prima verifica passata sulla migr 0137 |
| **Test** | `tests/loyalty_smoke.mjs` (8 probe di produzione in sola lettura, job `invarianti-produzione`, non bloccante) | login gate e `/club` text/html via App Proxy, 401 su firma assente e falsa sulle edge dirette, wrapper liquid di `loyalty-page`, `loyalty-orders` risponde JSON, il clone smoke resta 410 |
| **Test** | `tests/loyalty_guardie.mjs` (52 pin sul sorgente, job `test` bloccante, dal 2026-09-21 con i fix di Fase 6) | frammenti attesi dei fix: API `2026-07` e niente `2024-01` (T20), firma fresca (T1), PIN (T2), epoch/lotti/finestra (L1/L2/L8), RPC `loyalty_award_daily` e niente `award()` (L3), claim idempotente e idemp del proprietario (L4/L6, migr 0136), storno da due fonti (L5/G2), cinture di schema (T16), `rewards` con errori controllati (T9), guardaroba dal DB su `codice_norm` (S1/T33), escape e rete della pagina, `verify_jwt` in `config.toml`, una sola 0135/0136/0137 |
| **Non coperto** | concorrenza reale su `loyalty_award_daily` e rotazione della versione Admin API in `loyalty-proxy` | la RPC e' costruita per il caso (la seconda transazione rivaluta la guardia dopo il commit della prima) ma non e' stata provata con due richieste simultanee; la versione servita e' controllata a ogni giro da `loyalty-orders` (health warn), `loyalty-proxy` la usa solo nel ripiego del guardaroba |
