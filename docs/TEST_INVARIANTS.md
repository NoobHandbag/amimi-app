...
| Cosa | Dove | Nota |
|---|---|---|
| **Invariante** | tutto il repo | una modifica larga o un audit periodico passa tre cancelli: Gate 1 build offline (le suite di questa tabella + `node --check` sulle 19 edge + build e lint del web), Gate 2 audit a effort alto con un refuter indipendente per ogni finding A/B, Gate 3 pin (ogni finding chiuso o rinunciato per iscritto e fissato da un test). Skill `~/.claude/skills/amimi-audit-gate`; referto del primo giro completo `audits/AUDIT_GATE_2026-09-14.md` (14 aree, 138 finding: 7 A, ~75 B, ~55 C) |
...
| **Fatto (22-09, lead-outreach)** | `tests/lead_outreach_guardie.mjs` (41 asserzioni: blocco `PURE:lead-guard` ritagliato dal sorgente + regole su sorgente e migr 0139) | segnaposto rimasti, destinatario, opt-out, firma/opt-out garantiti, avvisi di contenuto (cotone/Italia, condizioni, allegati), giorno di Roma per il tetto, claim prima di Gmail, vincoli unici a DB, esito incerto e sblocco. Va aggiunto alla Gate 1 della skill |
