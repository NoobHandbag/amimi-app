...
| **Invariante** | RPC `loyalty_profile_save`, `loyalty_birthday_run`, `loyalty_bonus_second_run` (migr 0141, 22-09) | +10 profilo, +20 compleanno e +50 seconda borsa sono accrediti atomici con gate a DB (UPDATE sotto lock + UNIQUE parziale su `loyalty_events`; PK cliente+anno; PK cliente) e i giri sono NO-OP a flag spento; prova viva e senza residui in `tests/loyalty_profilo_idempotenza.sql` (blocco DO che si annulla: stesso salvataggio o stesso giro ripetuto = 1 evento, 29/02 -> 28/02, 30 giorni di anticipo, flag a lista, since non retroattivo, storno una volta sola, drift 0 a ogni passo), verde il 22-09 |
| **Test** | `tests/loyalty_guardie.mjs` (95 pin sul sorgente, job `test` bloccante, dal 2026-09-21 con i fix di Fase 6 e dal 22-09 con i pin 44-95 delle migr 0141/0142 e di `loyalty-proxy` v12) | frammenti attesi dei fix: API `2026-07` e niente `2024-01` (T20), firma fresca (T1), PIN (T2), epoch/lotti/finestra (L1/L2/L8), RPC `loyalty_award_daily` e niente `award()` (L3), claim idempotente e idemp del proprietario (L4/L6, migr 0136), storno da due fonti (L5/G2), cinture di schema (T16), `rewards` con errori controllati (T9), guardaroba dal DB su `codice_norm` (S1/T33), escape e rete della pagina, `verify_jwt` in `config.toml`, una sola 0135/0136/0137 |
