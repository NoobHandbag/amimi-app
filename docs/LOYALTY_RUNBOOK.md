...
Il pool si consuma un codice per riscatto (`loyalty_claim_code`, idempotente). Allarme `loyalty_pool` warn sotto `loyalty_pool_min` (20), error a 0 con riscatto ON. Stima del fabbisogno: `select count(*) from loyalty_points where points >= (select min(cost_points) from loyalty_rewards where active)` (i membri che possono riscattare oggi: 34 con `tessera` a 200, 467 con `tessera12` a 100) piu' i riscatti attesi nella settimana di campagna. Con la Parte D (§12) il lotto nuovo e' al 12% (`percentage: 0.12`, titolo `Premia - Tessera 12% (lotto <data>) PREMIA-XXXXXX`, `reward_key = 'tessera12'` nell'insert) e va caricato PRIMA di accendere `tessera12`, altrimenti i riscatti restano `pending`.
...
| D Premio unico 100 pt = 12% (migr 0142) | riga `loyalty_rewards` (`tessera12` attiva, `tessera` spenta) | sostituisce 200 pt = 20%. La riga `tessera12` (100 pt, `percentage` 12) esiste gia' SPENTA; `loyalty_redeem`/`loyalty_claim_code` leggono costo e valore dalla riga attiva (nessun 200 fisso nel codice: verificato), la pagina deriva i 10 timbri dal costo del premio attivo (10 punti a timbro con `tessera12`) e mostra `label` e `value` della riga. Chi ha 200+ punti riscatta piu' volte, un codice per ordine. KPI `v_loyalty_redeem_rate.membri_con_tessera_piena` segue il costo attivo (oggi 34 a 200; 467 a 100). **Prima di accendere**: pool di codici 12% caricato (§4: `reward_key = 'tessera12'`, titolo `Premia - Tessera 12% (lotto <data>)`, Shopify = scrittura solo con OK owner, stima 467 + margine); **dopo** l'accensione: togliere i 61 codici 20% liberi dal pool e disattivarli in Shopify (OK owner; `PREMIA-T20-6KQ4ZM` resta come storico). Provato in `tests/loyalty_profilo_idempotenza.sql` (108 punti -> codice e saldo 8, stesso idemp = stesso codice, 99 punti = rifiuto, premio spento = `reward_unknown`) | `loyalty_rewards`, RPC esistenti; accensione: `update loyalty_rewards set active = (key = 'tessera12') where key in ('tessera','tessera12');` rollback: lo stesso con `'tessera'` |
