...
1. **Retroattivo prima del 2026-07-23 (531 ordini):** il token dell'app custom non ha `read_all_orders`, quindi l'API restituisce ZERO ordini oltre i 60 giorni (nessun errore: `fetched: 0`). Aggiungere lo scope nell'admin Shopify (Impostazioni -> App e canali di vendita -> Sviluppa app -> app del token -> Configurazione -> ambiti Admin API -> `read_all_orders` -> Salva; se Shopify rigenera il token, aggiornare `app_config.shopify_token` da canale sicuro). Poi: `diag` deve elencare `read_all_orders`; backfill a chunk mensili `since 2026-02-01 .. until 2026-07-23` con `dryRun` prima (§5); rileggere `select count(*), sum(points) from loyalty_points` e `v_loyalty_ledger_drift`.
...
Leggere in risposta `fetched`, `candidati`, `credited`, `added`, `skipped` (gia' accreditati o a 0 punti), `resto`, `reversed_*`, `errors`, `api_version`; in `health_log` le chiavi `loyalty_orders` (giro), `loyalty_backfill`, `loyalty_refunds`. Un secondo giro identico deve dare `credited: 0`. Il giro normale legge gli ordini AGGIORNATI da min(ora - 3 giorni, ultimo giro - 1h) e accredita solo quelli creati dopo il watermark, fino a 100 per giro (il resto al giro dopo).
...
| Scope del token cambiato | `diag` non elenca `read_orders`/`read_customers`, giro con `Shopify HTTP 4xx` | ripristinare gli scope dell'app custom; niente `read_all_orders` = solo 60 giorni di ordini (`fetched: 0` sul passato NON e' "nessun ordine") |
