...
- Guardie: `tests/loyalty_guardie.mjs` (83 pin sul sorgente, bloccante), `tests/loyalty_smoke.mjs` (9 probe di produzione in sola lettura) e `tests/loyalty_profilo_idempotenza.sql` (prova viva delle RPC della 0141 sull'account di test, in un blocco che si annulla da solo: esito atteso `TEST_OK`).
...
| C Bonus seconda borsa | `loyalty_bonus_second_enabled` | +50 (`bonus_second`) una volta per cliente quando il 2° ordine accreditato (data ordine da `shopify_orders.created_at_shop`) arriva entro 90 giorni dal 1° ed e' creato dopo `loyalty_bonus_second_since`; se `since` e' vuoto il primo giro a flag acceso lo fissa a "adesso" (non retroattivo; per un'altra data scriverla a mano in ISO). Se il 2° ordine viene rimborsato per intero il bonus si storna (`bonus_second_reversal`, mai sotto zero, riprova al giro dopo). Pagina: "Seconda borsa entro il <data>: +50 punti" per chi ha 1 ordine e finestra aperta (oggi 360 membri) | tabella `loyalty_bonus_second`; vista `v_loyalty_second_order`; RPC `loyalty_bonus_second_run(p_max=200)`; cron `loyalty-bonus-second-daily` 06:25 UTC |
