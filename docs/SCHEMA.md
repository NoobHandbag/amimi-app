...
- **`loyalty_profiles`, `loyalty_birthday_awards`, `loyalty_bonus_second` (migr 0141, brief M4b 22-09, stesso sottosistema, tre flag OFF):** `loyalty_profiles` (`shopify_customer_id` PK, `birth_day` 1-31, `birth_month` 1-12, **nessun anno di nascita**, `birth_set_at`, `materiale_preferito` con check su lista chiusa, `consenso_compleanno` + `consenso_at`, `completed_at` = gate del +10); `loyalty_birthday_awards` (PK cliente + anno: +20 una volta l'anno); `loyalty_bonus_second` (PK cliente: +50 una volta, `points_reversed` per lo storno). Indici UNIQUE parziali su `loyalty_events` per source `profile_complete` (cliente), `bonus_second` (cliente), `birthday` (cliente + `meta->>'year'`). Viste: `v_loyalty_members` + `punti_cumulati`/`tier_cumulato` (colonne in coda), `v_loyalty_second_order` (primo e secondo ordine accreditato per cliente, finestra 90 giorni, data da `shopify_orders.created_at_shop`). RPC: `loyalty_profile_state`, `loyalty_profile_save`, `loyalty_birthday_run`, `loyalty_bonus_second_run`, `loyalty_flag_on`, `loyalty_points_cumulati`, `loyalty_tier_of`, `loyalty_birthday_in_year`. Stessa postura RLS/REVOKE; `ask_ro` legge `loyalty_profiles` a colonne SENZA giorno e mese (e per questo l'evento `birthday` porta nel `meta` solo l'anno: `loyalty_events.meta` e' leggibile da `ask_ro`). Migr 0142 (Gate 2 + Parte D): `v_loyalty_second_order.deadline` e' una `date` Europe/Rome e `entro_90gg` confronta giorni; riga `loyalty_rewards` `tessera12` (100 pt, 12%, `active = false`, accensione = scambio di `active` con `tessera`); `v_loyalty_redeem_rate.membri_con_tessera_piena` segue il costo del premio attivo. Runbook `LOYALTY_RUNBOOK.md` §12.
