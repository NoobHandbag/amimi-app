...
- **Migr 0145 (25-09), contenuti dell'outreach**: `lead_sequences` riscritte (IT 0-4, EN 1-4: link alla line sheet con `{{linesheet}}`, niente allegati, niente "come anticipato", cotone mai accostato all'Italia, niente minimi d'ordine ne' conto vendita con numeri); `lead_knowledge` popolata con 5 voci verificate (line sheet online senza prezzi, listino approvato ma NON pubblicato per decisione owner del 25-09: niente prezzi wholesale nelle email, condizioni NON definite, showroom e contatti, materiali e origine; insert rieseguibile, salta i titoli presenti); `app_flags.lead_linesheet_url` = pagina amimi-trade.
