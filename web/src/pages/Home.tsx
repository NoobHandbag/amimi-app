...
const TC: Record<string, [string, string]> = {
  bag: ['--negative-tint', '--negative-ink'], rocket: ['--negative-tint', '--negative-ink'],
  chart: ['--brand-tint', '--brand-hover'], plus: ['--brand-tint', '--brand-hover'],
  box: ['--brand-tint', '--brand-hover'], inbox: ['--brand-tint', '--brand-hover'],
  table: ['--brand-tint', '--brand-hover'], tag: ['--brand-tint', '--brand-hover'],
  count: ['--accent-tint', '--accent-ink'], search: ['--accent-tint', '--accent-ink'],
  store: ['--accent-tint', '--accent-ink'],
  euro: ['--warning-tint', '--warning-ink'], pulse: ['--warning-tint', '--warning-ink'], sparkles: ['--warning-tint', '--warning-ink'],
  recycle: ['--positive-tint', '--positive-ink'], handshake: ['--positive-tint', '--positive-ink'],
  return: ['--negative-tint', '--negative-ink'],
};
const tileTint = (icon: string) => TC[icon] ?? ['--brand-tint', '--brand-hover'];
  ...
  return (
    <div className="screen">
      ...
      <div className="ds-search">
        ...
        {q ? <button type="button" onClick={() => setQ('')} className="drawerx" style={{ width: 30, height: 30, fontSize: 13 }} aria-label="Pulisci"><Icon name="x" size={13} /></button> : null}
      </div>
          ...
          {cfg.finance && fin && (
            <button className="ds-hero" onClick={() => go('cruscotto')} type="button">
              <div>
                ...
                {fin.deltaPct !== null && (
                  <span className="chip"><Icon name={fin.deltaPct >= 0 ? 'arrow-up' : 'arrow-down'} size={12} /> {fin.deltaPct >= 0 ? '+' : ''}{fin.deltaPct}% vs {meseNome(nowMonth() - 1)}</span>
                )}
                {fin.spark.length > 1 && (
                  <svg className="spark" width="150" height="26" viewBox="0 0 150 26" preserveAspectRatio="none" aria-hidden="true">
                    <polyline fill="none" stroke="var(--on-brand)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" points={sparkPoints(fin.spark)} />
                  </svg>
                )}
              </div>
              <div className="sec">
              ...
              <span className="go">Cruscotto <Icon name="chevron-right" size={13} /></span>
            </button>
          )}
          ...
          {altro.length > 0 && (
            <section style={{ marginTop: 6 }}>
              <button type="button" className="ds-more" onClick={toggleAll}>
                ...
                <b>{showAll ? <>Nascondi <Icon name="chevron-up" size={13} /></> : <>Vedi tutte <Icon name="chevron-right" size={13} /></>}</b>
              </button>
